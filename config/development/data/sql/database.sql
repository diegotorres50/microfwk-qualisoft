...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 21-01-2017 a las 21:33:35
...
CREATE DEFINER=`diego_torres`@`localhost` PROCEDURE `procedure_auth_login` (IN `_userId` VARCHAR(20), IN `_userPass` VARCHAR(20), OUT `_responseCode` INTEGER, OUT `_responseMessage` VARCHAR(200), OUT `_userName` VARCHAR(200))  BEGIN
  ...
  /*Definicion de excepcion en caso de no encontrar resultados la consulta*/
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET _responseCode = UNSUCCESS_RESPONSE_CODE, _responseMessage = UNSUCCESS_RESPONSE_MESSAGE;
    ...
    /*Consultamos si el usuario y clave son validos*/
  SELECT
    /*Codigo de respuesta generico*/
    SUCCESS_RESPONSE_CODE,
        /*Mensaje de respuesta generico*/
        SUCCESS_RESPONSE_MESSAGE,
        /*Todos los camos que quiera retornar en la respuesta*/
    users.user_name

    /*Guardamos los valores en variables de salida*/
  INTO
    _responseCode,
        _responseMessage,
    _userName
    ...
    users.user_id = _userId AND
        /*El usuario no este purgado*/
        users.purged != 1 AND
        /*La clave se encripta 2 veces en la base de datos*/
...
--
-- Creación: 21-01-2017 a las 19:24:00
--
...
--
-- Metadatos para view_users
--

--
-- Metadatos para view_users_x_centers
--
